...
DMD::DMD(byte panelsWide, byte panelsHigh)
{
    ...
    // Set R_DATA to HIGH immediately
    GPIO.out_w1ts = (1 << PIN_DMD_R_DATA);

    // Set A, B, CLK, SCLK, and nOE to LOW immediately (Combined)
    GPIO.out_w1tc = (1 << PIN_DMD_A) | (1 << PIN_DMD_B) | (1 << PIN_DMD_CLK) | 
                    (1 << PIN_DMD_SCLK) | (1 << PIN_DMD_nOE);
